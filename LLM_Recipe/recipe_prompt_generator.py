...
def recipe_prompt_generator(input, chatInput, outputFormat):
    prompt = chat_prompt.format_prompt(pantry = input["pantry"], diet = input["diet"], cuisine = input["cuisine"], prevRecipe = input["prevRecipe"], feedback = input["feedback"] , chatInput = chatInput, outputFormat = outputFormat).to_messages()
    return prompt